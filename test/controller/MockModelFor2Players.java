...
/**
 * Mock Model when 2 players are there in the game.
 */
public class MockModelFor2Players implements IgameState {
  private Appendable output;
  private int turn = 0;

  /**
   * Constructor of the mock model for 2 players.
   * @param output output
   */
  public MockModelFor2Players(Appendable output) {
    this.output = output;
  }
  ...
  @Override public int getTurn() {
    try {
      output.append("getTurn method called player ").append(String.valueOf(turn));
    } catch (IOException e) {
      e.printStackTrace();
    }
    return 0;
