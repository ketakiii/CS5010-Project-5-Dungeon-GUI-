...
public class Grid {
  ...
  private void matrixToMst() {
    ...
    for (int[] i : connectedInMst) {
      ...
      location1.attachLocation(this.getDirection(coord1, coord2, this.dungeon[0].length,
          this.isWrapped), location2);
      location2.attachLocation(this.getDirection(coord2, coord1, this.dungeon[0].length,
          this.isWrapped), location1);
    }
  ...
  private Direction getDirection(int[] i, int[] j, int col, boolean isWrapped) {
    if (!isWrapped) {
      if (j[0] - i[0] > 0) {
      ...
      if ((i[0] - 1 + col) % col == j[0] && i[1] == j[1]) {
        this.direction = Direction.NORTH;
      } else if ((i[0] + 1 + col) % col == j[0] && i[1] == j[1]) {
        this.direction = Direction.SOUTH;
      } else if (i[0] == j[0] && (i[1] - 1 + col) % col == j[1]) {
        this.direction = Direction.WEST;
      } else if (i[0] == j[0] && (i[1] + 1 + col) % col == j[1]) {
        this.direction = Direction.EAST;
      } else {
  ...
  /**
   * The method returns the distance n away from the current location.
   * @param location location
   * @param random random
   * @return location
   */
  public Ilocation getNdistanceAwayLocation(Ilocation location, Random random) {
    List<Ilocation> caveList1 = this.getAllCaves();
    this.neighbourList.clear();
    getNnearestNeighbours(location, 2);
    List<Ilocation> caveList2 = new ArrayList<>();
    ...
    int randomIndex = this.helperInit(0, caveList2.size(), random);
    return caveList2.get(randomIndex);
  }
